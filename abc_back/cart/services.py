...
class CartService:
    ...
    def add_item(self, session_key: str, sub_product_id: Id, quantity: int) -> CartItem:
        cart = self._cart_repository.get_or_create_cart(session_key)
    ...
    def change_item_quantity(self, session_key: str, item_id: Id, quantity: int):
        cart = self._cart_repository.get_or_create_cart(session_key)
    ...
    def remove_item(self, session_key: str, item_id: Id):
        cart = self._cart_repository.get_cart(session_key)
        item = self._cart_repository.get_cart_item_by_id(cart.id, item_id)
        if item.cart != cart:
            raise BadRequest("Товар не в корзине")
        item.delete()

    def get_or_create_cart(self, session_key: str | None = None):
        return self._cart_repository.get_or_create_cart(session_key)

    def clear_cart(self, session_key: str):
        cart = self._cart_repository.get_cart_by_session_key(session_key)
        if cart:
